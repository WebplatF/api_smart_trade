@php
    if (!function_exists('formatINR')) {
        function formatINR($num)
        {
            ...
            $formatted = ($negative ? '-' : '') . $other . $lastThree;
    ...
    if (!isset($invoice)) {
        $invoice = [
            'invoiceNo'    => 'SMTA-IN-001',
            'invoiceDate'  => '22 July 2026',
            'invoiceTerms' => '50% Initial',
            'customer'     => ['name' => 'Arun Kumar', 'mobile' => '9876543210', 'email' => 'arun@example.com'],
            'items'        => [
                ['description' => 'First Installment is paid.', 'qty' => 1, 'amount' => 10000],
                ['description' => 'Second Installment is paid.', 'qty' => 1, 'amount' => 5000],
                ['description' => 'Final Installment is paid.', 'qty' => 1, 'amount' => 2500],
            ],
            'cgstRate'     => 9,
            'sgstRate'     => 9,
        ];
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .header-table .logo-cell img{ width: 210px; }
  .header-table .tag-cell{ text-align: right; }
  .invoice-tag{
  ...
  .bill-to .field{ font-size: 11px; margin: 6px 0; line-height: 1.4; }
  .bill-to .field .label{ font-weight: bold; color: #1a1a1a; }
  .bill-to .field .value{ color: #2f4f8f; font-weight: 600; }

  .invoice-info{ text-align: right; font-size: 11px; }
  .invoice-info .row{ margin: 6px 0; line-height: 1.4; }
  .invoice-info .row .label{ font-weight: bold; color: #1a1a1a; }
  .invoice-info .row .value{ font-weight: bold; color: #2f4f8f; margin-left: 8px; }
...
.invoice-table td:nth-child(4) {
  ...
  .sl-cell{ text-align: center; color: #000000; font-weight: bold; padding-left: 8px; padding-right: 8px; }
  .desc-cell{ text-align: left; line-height: 1.5; }
  .top-value{ text-align: center; padding-top: 12px; }
  .top-value.qty-accent{ color: #000000; font-weight: bold; padding-left: 8px; padding-right: 8px; }

  .filler-row td{ height: 250px; }
    ...
    <tr>
      ...
      <td style="width: 45%;">
        <div class="invoice-info">
          ...
          <div class="row"><span class="label">Invoice Terms:</span><span class="value">{{ $invoiceTerms }}</span></div>
        </div>
      </td>
    </tr>
  </table>
